...
namespace SalesAnalyzer.Application.Converters
{
    public class SaleItemCustomConverter : ConverterBase
    {
        ...
        {
            var regex = new Regex("^\\[.+\\]$");
            if (!regex.IsMatch(from)) throw new Exception("Sale Item must start and terminate with brackets: " + from);
        ...
        private SaleItem DeserializeItem(string item)
        {
            ...
            if (values.Length != 3) throw new Exception("Sale item must have 3 values: " + item);

            var numberFormatInfo = new NumberFormatInfo {CurrencyDecimalSeparator = "."};

            return new SaleItem(values[0], double.Parse(values[1], numberFormatInfo),
                double.Parse(values[2], numberFormatInfo));
        }
    }
}